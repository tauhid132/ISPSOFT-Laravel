@extends('master')
@section('title','Monthly Upstream Bills | ATS Technology')
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0">Monthly Upstream Bills</h4>
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                <li class="breadcrumb-item"><a href="javascript: void(0);">Accounts</a></li>
                                <li class="breadcrumb-item active">Monthly Upstream Bills</li>
                            </ol>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0">
            <div class="modal-header p-3 bg-soft-info">
                <h5 class="modal-title" id="modalHeader">Edit Upstream Bill</h5>
...
<div class="modal fade zoomIn" id="payBillModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0">
            <div class="modal-header p-3 bg-soft-info">
                <h5 class="modal-title" id="modalHeader">Pay Upstream Bill</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" id="close-modal"></button>
            </div>
...
<script>
    ...
    var dataTable = $('#scroll-horizontal').DataTable({
        ...
        "ajax":{
            "url": "{{ route('getMonthlyUpstreamBills') }}",
            "dataType": "json",
            "type": "POST",
            "data": function(d){
    ...
    $(document).on('click', '.edit_bill', function(){
        var id = $(this).attr("id");  
        $.ajax({  
            url:"{{ route('fetchMonthlyUpstreamBillSingle') }}",  
            method:"post",  
            data:{id:id},  
            success:function(data){ 
                $('#id').val(data.id);
                $('#upstream_name').val(data.upstream.upstream_name);
                $('#bill').val(data.bill);
    ...
    $(document).on('click', '.pay_bill', function(){
        ...
        $.ajax({  
            ...
            success:function(data){ 
                $('#id2').val(data.id);
                $('#upstream_name2').val(data.upstream.upstream_name);
