@extends('master')
@section('title','Reseller Packages | ATS Technology')
@section('main-body')
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0"><i class="fa fa-store"></i> Reseller Packages</h4>
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                <li class="breadcrumb-item"><a href="javascript: void(0);">Reseller</a></li>
                                <li class="breadcrumb-item active">Reseller Packages</li>
                            </ol>
                        </div>
                    </div>
                </div>
            ...
            <form id="add_edit_package_form">
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-lg-4">
                            <div>
                                <label for="name" class="form-label">Package Bandwidth</label>
                                <input type="text" name="package_bandwidth" class="form-control" id="package_bandwidth" required />
                            </div>
                        </div>
                        <div class="col-lg-4">
                            <div>
                                <label for="name" class="form-label">Package Bill</label>
                                <input type="text" name="bill" class="form-control" id="bill" required />
                            </div>
                        </div>
...
<script>
    ...
    $('#add_edit_package_form').on("submit", function(event){ 
        ...
        $.ajax({  
            url:"{{ route('addEditResellerPackage') }}",  
            method:"POST",  
    ...
    $(document).on('click', '.edit_package', function(){
        var id = $(this).attr("id");  
        $.ajax({  
            url:"{{ route('fetchResellerPackage') }}",  
            method:"post",  
            data:{id:id},  
            success:function(data){ 
                $('#id').val(data.id);
                $('#package_name').val(data.package_name);
                $('#package_bandwidth').val(data.package_bandwidth);
                $('#bill').val(data.bill);
    ...
    $(document).on('click', '.delete_package', function(){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url:"{{ route('deleteResellerPackage') }}",
                    method:"POST",
                    data:{id:id},
                    success:function(data){
